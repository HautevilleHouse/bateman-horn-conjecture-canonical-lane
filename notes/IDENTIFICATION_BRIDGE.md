...
In-paper anchor: `paper/BATEMAN_HORN_CONJECTURE_PREPRINT.md` (bridge and margin interface).

## Goal
Identify the canonical endpoint representative with the ordinary problem-side target for `proving persistence of the Bateman-Horn prime-density law for admissible polynomial families through an admissible prime-pattern density closure architecture`.
...
2. the endpoint-transfer lock `horn_transfer` identifies the surviving representative;
3. the coherence remainder `eps_coh` is not smuggled into the endpoint;
4. the strict closure margin `M_BH` remains positive.
...
Payload: verify `horn_transfer` against the constants registry and runtime certificate.
...
If lock persistence, determining-class uniqueness, and strict coherence hold, then the endpoint representative is identified and the final margin `M_BH` is meaningful.

## Current Instantiation

- endpoint-transfer artifact key: `horn_transfer`
