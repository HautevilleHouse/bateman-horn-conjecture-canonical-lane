...
In-paper anchor: `paper/BATEMAN_HORN_CONJECTURE_PREPRINT.md` (`BH_G3`).

## Goal
Make the compactness/no-Zeno gate explicit for `proving persistence of the Bateman-Horn prime-density law for admissible polynomial families through an admissible prime-pattern density closure architecture`: near-failure behavior must be captured, not allowed to drift or accumulate invisibly.
...
`BH_G3` closes when `kappa_compact` provides compactness/no-collapse control: normalized near-failure families are precompact and admissible windows do not collapse.
This is the first-failure capture contribution to `M_BH`.
...
Payload: check that `kappa_compact` is registered and participates in the runtime certificate.
...
If near-failure extraction and no-Zeno spacing hold, then `BH_G3` prevents hidden drift before rigidity is applied.
