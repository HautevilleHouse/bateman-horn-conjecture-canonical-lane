#!/usr/bin/env python3
"""Canonical-lane closure guard for the bateman horn conjecture workspace."""
...
def compute_report(data: dict[str, Any], sigma_star: float | None, strict_coh_zero: bool) -> dict[str, Any]:
    ...
    native_names = ['BH_G1', 'BH_G2', 'BH_G3', 'BH_G4', 'BH_G5']
    gates = {name: ('PASS' if passed else 'FAIL') for name, passed in zip(native_names, gate_pass)}
    gates['BH_G6'] = 'PASS' if coh_pass else 'FAIL'
    gates['BH_GM'] = 'PASS' if gm_pass else 'FAIL'
    blockers = {}
    ...
    blockers['BH_G6'] = [] if coh_pass else ['eps_coh missing/invalid or strict-zero target failed']
    blockers['BH_GM'] = [] if gm_pass else ['strict margin <= 0 or upstream gates failed']
    normalized_gates = {'G1': gates['BH_G1'], 'G2': gates['BH_G2'], 'G3': gates['BH_G3'], 'G4': gates['BH_G4'], 'G5': gates['BH_G5'], 'G6': 'NA', 'GCoh': gates['BH_G6'], 'GM': gates['BH_GM']}
    normalized_blockers = {'G1': blockers['BH_G1'], 'G2': blockers['BH_G2'], 'G3': blockers['BH_G3'], 'G4': blockers['BH_G4'], 'G5': blockers['BH_G5'], 'G6': [], 'GCoh': blockers['BH_G6'], 'GM': blockers['BH_GM']}
    report = {'meta': {'computed_at_utc': dt.datetime.now(dt.timezone.utc).replace(microsecond=0).isoformat(), 'framework': 'bateman_horn_conjecture'}, 'schema': {'normalized_gate_keys': ['G1', 'G2', 'G3', 'G4', 'G5', 'G6', 'GCoh', 'GM'], 'g6_policy': 'NA means this framework has no standalone pre-coherence G6 gate'}, 'lane': {'canonical_theorem_lane': 'manifold_constrained', 'active_lane': 'manifold_constrained'}, 'inputs': {**primary_values, 'eps_coh': eps_coh, 'sigma_star_can': sigma_star}, 'derived': {'strict_margin': margin, 'strict_coh_zero': bool(strict_coh_zero)}, 'gates': gates, 'blockers': blockers, 'normalized': {'gates': normalized_gates, 'blockers': normalized_blockers}}
